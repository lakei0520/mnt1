...
# 存储第二近左侧的直线和第二近右侧的直线
second_closest_left_line = None
second_closest_right_line = None
...
for line in lines:
    ...
    if line_center_x < center_x and distance_to_center < min_left_distance:
        second_closest_left_line = closest_left_line
    ...
    # 更新最近右侧直线的信息
        ...
        second_closest_right_line = closest_right_line
...
# 绘制第二近左侧的直线
if second_closest_left_line is not None and min_left_distance > 10:
    x1, y1, x2, y2 = second_closest_left_line[0]
    dx, dy = x2 - x1, y2 - y1
    extended_x1 = int(x1 - extension_length * dx / np.sqrt(dx**2 + dy**2))
    extended_y1 = int(y1 - extension_length * dy / np.sqrt(dx**2 + dy**2))
    extended_x2 = int(x2 + extension_length * dx / np.sqrt(dx**2 + dy**2))
...
# 绘制第二近右侧的直线
if second_closest_right_line is not None and min_right_distance > 10:
    x1, y1, x2, y2 = second_closest_right_line[0]
    dx, dy = x2 - x1, y2 - y1
    extended_x1 = int(x1 - extension_length * dx / np.sqrt(dx**2 + dy**2))
    extended_y1 = int(y1 - extension_length * dy / np.sqrt(dx**2 + dy**2))
    extended_x2 = int(x2 + extension_length * dx / np.sqrt(dx**2 + dy**2))
